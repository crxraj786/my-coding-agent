...
export default function AdminManager() {
  ...
  const handleBalanceAdjust = async () => {
    ...
    try {
      ...
      const newBal = balType === 'credit' ? currentBal + amt : currentBal - amt;
      if (newBal < 0) {
        toast({
          title: 'Error',
      ...
      await updateAdmin(selAdmin.adminId, { balance: newBal });
  ...
  return (
    <div className="space-y-5">
            ...
            <div className="space-y-4 mt-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  className="h-11 rounded-xl glass text-white placeholder:text-white/25 focus:border-[#09D1C7]/40"
                />
                <p className="text-xs text-white/25">Min: 1,000 &middot; Max: 100,000</p>
              </div>
              <Button
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {admins.map((admin) => (
            <div
              key={admin.id}
              className="glass rounded-2xl p-5 transition-all duration-200 hover:border-white/15"
            >
              {/* Card Header */}
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 w-8 p-0 text-white/30 hover:text-white/60 hover:bg-white/10 rounded-lg"
                      >
                        <MoreVertical className="w-4 h-4" />
                      </Button>
                    </DropdownMenuTrigger>
                    <DropdownMenuContent
                      className="glass-strong rounded-xl border-white/10"
                      align="end"
                    >
                      <DropdownMenuItem
                        onClick={() => {
              ...
              <div className="space-y-2.5 text-xs">
                ...
                <div className="flex justify-between">
                  <span className="text-white/35">Initial</span>
                  <span className="text-white/50">
                    {admin.initialBalance || 0}
                  </span>
                </div>
      ...
      <Dialog open={balanceOpen} onOpenChange={setBalanceOpen}>
        <DialogContent className="glass-strong rounded-2xl sm:max-w-md border-white/10">
          <DialogHeader>
            <DialogTitle className="text-white text-lg">Adjust Balance</DialogTitle>
          </DialogHeader>
          {selAdmin && (
            <div className="space-y-4 mt-2">
              ...
              <div className="space-y-2">
                ...
                >
                  <SelectTrigger className="h-11 rounded-xl glass text-white">
                    <SelectValue />
                  </SelectTrigger>
              ...
              <div className="flex gap-3 pt-1">
                <DialogClose asChild>
                  <Button className="flex-1 h-11 glass rounded-xl text-white/60 hover:text-white hover:bg-white/10">
                    Cancel
                  </Button>
                </DialogClose>
                <Button
                  ...
                  disabled={adjusting}
                  className="flex-1 h-11 btn-gradient rounded-xl text-sm"
                >
                  {adjusting ? (
                    <Loader2 className="w-4 h-4 animate-spin" />
                  ) : (
                    `${balType === 'credit' ? 'Credit' : 'Debit'}`
                  )}
                </Button>
              </div>
            </div>
        ...
        <AlertDialogContent className="glass-strong rounded-2xl border-white/10">
          <AlertDialogHeader>
            ...
            <AlertDialogDescription className="text-white/50">
              {confirm?.type === 'delete'
                ? `Delete "${confirm?.admin.displayName}"? This will remove the admin and all associated keys.`
